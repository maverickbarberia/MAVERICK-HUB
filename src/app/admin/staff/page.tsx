  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Directorio de Personal</h1>
          <p className="text-gray-400 text-sm mt-1">
            Gestiona el equipo administrativo y de soporte ({staff?.length || 0} registrados).
          </p>
        </div>
        <div className="w-full md:w-auto">
          <AddTeamMemberModal role="STAFF" />
        </div>
      </div>
      ...
      {error ? (
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl flex items-center gap-3">
          <ShieldAlert />
          <p>La tabla de personal no existe aún. Por favor crea la tabla <code>team_members</code> en Supabase.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
          {staff?.map((member) => (
            ...
            >
              ...
              <div className="mt-5 pt-4 border-t border-white/5">
                <form action={toggleTeamMemberStatus.bind(null, member.id, member.status)}>
          ...
          {(!staff || staff.length === 0) && (
            <div className="col-span-full py-12 text-center backdrop-blur-xl bg-white/2 border border-white/5 rounded-3xl">
              <UserCheck className="mx-auto h-12 w-12 text-gray-500 mb-4" />
              <h3 className="text-lg font-medium text-white">No hay personal</h3>
              <p className="text-gray-400 mt-1">Registra a tu primer miembro del staff para empezar.</p>
            </div>
          )}
        </div>
      )}
